        ...
        for sd in unique_search_dirs:
            ...
            for existing in sd.glob(f"[0-9][0-9][0-9]_{suffix}"):
                if existing.exists() and existing.resolve() != target_path.resolve():
                    print(f"  ↗️ Correcting item number/location: {existing.name} (in {sd.name}) → {target_name}")
                    try:
...
def download_video(video_url: str, filepath: Path, cookies: list = None, download_dir: Path = None) -> bool:
