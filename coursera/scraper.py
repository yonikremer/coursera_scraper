...
class CourseraScraper:
    ...
    def __init__(self, email: str, download_dir: str = "coursera_downloads", headless: bool = False):
        ...
        self.auth = Authenticator(self.driver, self.session, self.email, self.download_dir)
        self.asset_manager = AssetManager(self.shared_assets_dir, self.session, self.driver)
    ...
    def _process_course_item(self, item_url: str, course_dir: Path, module_dir: Path,
        ...
        try:
            ...
            if len(existing_items) > 0:
                ...
                for item in existing_items:
                    ...
                    if len(item.name) > 4 and item.name[3] == '_' and not item.name.startswith(f"{item_counter:03d}_"):
                        ...
                        item_file = get_or_move_path(course_dir, module_dir, target_filename)
            ...
            if item_type == "video":
                downloaded_something, count = self.video_extractor.process(course_dir, module_dir, item_counter, title, item_url, self.browser)
                materials_downloaded += count

            if item_type == "reading":
                downloaded_something, count = self.reading_extractor.process(course_dir, module_dir, item_counter, title, downloaded_files)
                materials_downloaded += count

            if item_type in ["quiz", "assignment"]:
                downloaded_something, count = self.quiz_extractor.process(course_dir, module_dir, item_counter, title, item_type)
                materials_downloaded += count
            ...
            _, pdf_count = extract_pdfs(self.driver, course_dir, module_dir, item_counter, downloaded_files, self.session)
