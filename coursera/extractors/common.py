...
class AssetManager:
    ...
    def localize_css_assets(self, css_text: str, css_file_path: Path) -> str:
        ...
        url_pattern = re.compile(
            rf'url\s*\(\s*(["\']?)(https?://.*?)\1\s*\)', re.IGNORECASE
        )
        ...
        def replace_url(match):
            ...
            try:
                ...
                base_name = sanitize_filename(
                    Path(url.split("/")[-1].split("?")[0]).stem
                )
                if not base_name or len(base_name) < 2:
                    base_name = "asset"

                filename = (
                    f"{base_name}_{url_hash}.{ext}"
                    if ext
                    else f"{base_name}_{url_hash}"
                )
    ...
    def download_course_css(self, item_dir: Path = None) -> str:
        ...
        try:
            css_elements = self.driver.find_elements(
        ...
        try:
            ...
            for idx, style in enumerate(style_elements):
                try:
                    ...
                    if not css_text or len(css_text.strip()) < 20:
                        continue

                    # Hash the content to avoid duplicates across pages.
                    content_hash = hashlib.md5(
                        css_text.encode("utf-8", errors="ignore")
                    ).hexdigest()
    ...
    def localize_images(self, content_elem, item_dir: Path = None) -> int:
        ...
        try:
            ...
            if images:
                ...
                if item_dir:
                    try:
                        rel_images_base = os.path.relpath(
                            global_images_dir, item_dir
                        ).replace("\\", "/")
                    except ValueError:
                ...
                for img in images:
                    try:
                        ...
                        img_name = (
                            f"{sanitize_filename(base_name)}_{content_hash[:8]}.{ext}"
                        )
                        ...
                        self.image_url_to_path[src] = (
                            f"../../shared_assets/images/{img_name}"
                        )
                        self.driver.execute_script(
                            "arguments[0].setAttribute('src', arguments[1])",
                            img,
