...
class ReadingExtractor:
    ...
    def process(self, course_dir: Path, module_dir: Path, item_counter: int,
        ...
        try:
            ...
            new_downloads, url_to_local = self._download_attachments(course_dir, module_dir, item_counter, downloaded_files)
            ...
            if content and selector_used:
                ...
                try:
                    ...
                    downloaded_count += self.asset_manager.localize_images(content_elem, item_dir=module_dir)
                ...
                for a_tag in soup.find_all('a', href=True):
                    ...
                    if not data_name:
                        ...
                        if child_with_data: data_name = child_with_data.get('data-name')
                    ...
                    if not matched_file_name:
                        is_asset = any(pattern in href for pattern in ['api.coursera.org/api/asset/v1/', 'cloudfront.net', 'coursera-university-assets'])
                        if is_asset:
                            try:
                                url_filename = os.path.basename(urllib.parse.urlparse(href).path)
                                if url_filename:
                                    clean_url_name = sanitize_filename(urllib.parse.unquote(url_filename))
                                    filename = f"{item_counter:03d}_attachment_{clean_url_name}"
                                    attach_file = get_or_move_path(course_dir, module_dir, filename)
                                    if not attach_file.exists():
                                        print(f"  ⬇ Downloading asset link: {clean_url_name}")
...
<body>
    ...
    def _download_attachments(self, course_dir: Path, module_dir: Path, item_counter: int,
                             downloaded_files: Set[str]) -> Tuple[int, dict]:
        ...
        try:
            ...
            for attach_link in attachment_links:
                try:
                    ...
                    attach_url = attach_link.get_attribute('href') or attach_link.get_attribute('data-url')
