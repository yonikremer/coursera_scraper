...
def start_ollama_server():
    ...
    try:
        ...
        ollama_process = subprocess.Popen(
            ...
            creationflags=subprocess.CREATE_NEW_CONSOLE,  # Windows: new window/hidden
        )
...
def get_html_files(root_dir: str) -> List[str]:
    ...
    filtering out obvious quizzes/assignments by name.
    """
    html_files = []
...
def extract_text_from_html(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        # We look for <input type="radio">, <input type="checkbox">,
        # <textarea>, or specific quiz classes.
        ...
        if has_radio or has_checkbox or has_textarea or has_quiz_class:
            print(
                f"Skipping (detected quiz/interactive content): "
...
def generate_content_updates(
    ...
    *** REQUIRED OUTPUT FORMAT ***
    ...
    payload = {
        ...
        "prompt": prompt,
        "stream": False,
        "options": {"num_ctx": 4096, "temperature": 0.3},  # Reduced to fit in 6GB VRAM
    }
    ...
    try:
        ...
        if "|||SEPARATOR|||" in text_response:
            ...
            local_summary = (
                raw_summary.replace("```html", "").replace("```", "").strip()
            )
...
def summarize_file(file_path: str, context: str = "") -> Tuple[bool, str]:
    ...
    Returns (True, new_context) if successful or if summary already exists
    (no-op success), (False, context) otherwise.
    """
...
def process_course(course_name: str, files: List[str], pbar: tqdm):
    ...
    for file_path in files:
        ...
        local_summary_html, course_context = generate_content_updates(
            course_context, text, file_name
        )
...
def summarize_all_readings(root_dir: str = "coursera_downloads"):
    ...
    print(
        ...
        "(Note: The first file per course may take longer)"
    )

    try:
        with tqdm(total=len(files_to_process)) as pbar:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=len(course_files)
            ) as executor:
                futures = []
