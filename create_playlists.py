...
def write_wpl(playlist_path: Path, title: str, video_paths: list[Path], relative_to: Path = None):
    """
    ...
    with open(playlist_path, 'w', encoding='utf-8') as f:
        ...
        for vid in video_paths:
            ...
            xml_path = win_path.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
...
def create_playlists_for_course(course_dir: Path):
    ...
    modules = sorted([d for d in course_dir.iterdir() if d.is_dir()], key=module_sort_key)

    for module in modules:
        # Find all mp4 files in the module
        mp4s = sorted([f for f in module.iterdir() if f.is_file() and f.suffix.lower() == '.mp4'])
        
        if mp4s:
            # Create module-level playlist
            module_playlist_name = f"{module.name}.wpl"
            module_playlist_path = module / module_playlist_name
            module_title = f"{course_dir.name} - {module.name}".replace('_', ' ').title()
    ...
    if all_course_videos:
        ...
        write_wpl(course_playlist_path, course_title, all_course_videos, relative_to=course_dir)
        print(f"    [Course] Created {course_playlist_name} ({len(all_course_videos)} videos)")
    else:
