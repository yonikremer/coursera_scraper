...
def generate_sidebar_html(course_tree, current_file_path):
    sidebar_html = '<div id="sidebar"><div class="sidebar-header"><h2>Course Outline</h2></div>'
...
def process_html_file(file_path, course_tree):
    ...
    try:
        ...
        for style in soup.find_all('style'):
            if '#app-container' in style.string or '#sidebar' in style.string or '.content-wrapper' in style.string:
                style.decompose()
...
def generate_course_navigation(course_dir: Path):
    ...
    for module_dir in sorted([d for d in course_dir.iterdir() if d.is_dir()], key=lambda x: x.name):
        ...
        for f in files:
            if f.suffix == '.html' and f.name.endswith('_view.html'): continue # Skip old wrappers
...
def scan_and_generate(root_dir):
    ...
    for course_dir in sorted([d for d in root_dir.iterdir() if d.is_dir()]):
        if course_dir.name.startswith('.') or course_dir.name == 'shared_assets': continue
